...
function page() {
  ...
  const element = (id) => {
    if (!elements.has(id)) elements.set(id, {
      textContent: '', value: '1000', disabled: false,
...
test('Telegram link and QR follow the active session and disappear on Stop', async () => {
  const { element, run } = page();
  run(`
    video.srcObject = { getTracks: () => [] };
    api = async () => ({ session_id: 'new', predicate: 'present', direction: 'rising', telegram_link: 'https://t.me/cam?start=new', usage: {total: 0, usd: 0} });
  `);
  await run("start('arrives')");
  assert.equal(element('telegram').hidden, false);
  assert.equal(element('telegramLink').href, 'https://t.me/cam?start=new');
  assert.equal(element('telegramQr').src, '/session/new/qr.svg');
  run('stop()');
  assert.equal(element('telegram').hidden, true);
  assert.equal(element('telegramQr').src, undefined);
});
